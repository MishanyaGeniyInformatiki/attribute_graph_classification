...
token = 'f087ea8ef087ea8ef087ea8e6cf0f0831bff087f087ea8e90ffd0728cbe73e51829b35d'
community_id = 'miptfpmi'
...
def DictId(): # функция записывает в файл dict_id.json словарь, состоящий из уникальных значений id
    ...
    '''
    all_persons_list = list()
    for i in range(10):
        friends = get_person_friends(persons_list[i])
        if friends != 'This account is private':
            all_persons_list.append(persons_list[i])
            for j in range(len(friends)):
                all_persons_list.append(friends[j])

    all_unique_persons_list = list(set(all_persons_list))
    all_persons_dict = {all_unique_persons_list[i]: i for i in range(len(all_unique_persons_list))}
    with open('', 'wb') as out:
        pickle.dump(str(all_persons_dict), out)
    out.close()
    return all_persons_dict '''
...
#create_FriensIdTxt(fpmi_users_id)
#DictId_2()
