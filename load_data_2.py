...
class MiptFpmiDataset(DGLDataset):
    ...
    def process(self):
        ...
        edges_data = pd.read_csv('D:\Документы\Python_projects\data_2.csv')
        nodes_data = pd.read_csv('D:\Документы\Python_projects\_users_sex.csv')
        ...
        for i in range(vector.shape[0]):
            ...
            if len(train_nodes) < 4000: # набор для обучения
                if vector[i][2] == 1:
                    nosex_list.append(i) # номера позиций челов без пола
                    print(i)
                elif vector[i][0] == 1:
